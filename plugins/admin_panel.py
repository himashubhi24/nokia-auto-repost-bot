...
def admin_markup():
    return InlineKeyboardMarkup(
        [
            ...
            [
                InlineKeyboardButton("Add Force Sub", callback_data="admin_add_forcesub"),
                InlineKeyboardButton("Remove Force Sub", callback_data="admin_remove_forcesub"),
            ],
...
async def admin_callbacks(client: Client, query: CallbackQuery):
    ...
    if action == "admin_help_add":
        ...
        return
    elif action == "admin_add_forcesub":
        PENDING_ADMIN_INPUT[user_id] = "forcesub"
        await query.answer("Send force-sub channel ID")
        await query.message.reply_text("Send force-sub channel ID like <code>-1001234567890</code>. Bot must be admin there.")
        return
    elif action == "admin_remove_forcesub":
        PENDING_ADMIN_INPUT[user_id] = "remove_forcesub"
        await query.answer("Send force-sub channel ID")
        await query.message.reply_text("Send force-sub channel ID to remove like <code>-1001234567890</code>.")
        return
        ...
        if not channels:
            text = "No dynamic force-sub channels configured."
        else:
            text = "<b>Force-sub Channels</b>\n" + "\n".join(f"<code>{c['channel_id']}</code>" for c in channels)
    else:
...
@Bot.on_message(filters.private & filters.user(ADMINS) & filters.text & ~filters.command(["admin", "addchannel", "removechannel", "pausepair", "resumepair", "listchannels", "addforcesub", "removeforcesub", "setamazon", "stats", "broadcast", "users", "start"]))
async def admin_pending_input(client: Client, message: Message):
    ...
    try:
        ...
        if action == "forcesub":
            channel_id = int(text)
            await add_force_sub_channel(channel_id)
            PENDING_ADMIN_INPUT.pop(user_id, None)
            await message.reply_text("Force-sub channel added. Users will see it on /start before verification.", reply_markup=admin_markup())
            return
        if action == "remove_forcesub":
            channel_id = int(text)
            await remove_force_sub_channel(channel_id)
            PENDING_ADMIN_INPUT.pop(user_id, None)
            await message.reply_text("Force-sub channel removed.", reply_markup=admin_markup())
            return
        if action == "session":
            session = text
            if len(session) < 50:
